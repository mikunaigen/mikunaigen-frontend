...
@Component({
  selector: 'app-usuario-plan-seccion',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, NgIconComponent, ImagenPreviewComponent],
  templateUrl: './usuario-plan-seccion.component.html',
})
export class UsuarioPlanSeccionComponent implements OnInit, OnDestroy {
    ...
    this.planService.obtenerContexto().subscribe({
      ...
      error: (err) => {
        ...
        this.modal.set({
          tipo: 'error',
          titulo: 'Error',
          mensaje: err?.error?.message || 'No se pudo cargar la información del plan.',
        });
      },
    });
  }
